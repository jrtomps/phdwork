...
ClassImp(PPCoincMapRegions);

const Char_t*  PPCoincMapRegions::outfname_base = "pffrag2_hist_run";

//___________________________________________________________________________
PPCoincMapRegions::PPCoincMapRegions(UInt_t nregions, ConfigEntry *ce, bool save)
    : TSelector(),
    fPed(),
    fThresh(),
    fLowCut(),
    ...
    //   max_clock(),
{
    ...
    if (ce)
    {
        ...
        std::cout << "Error in <PPCoincMapRegions::PPCoincMapRegions> : Invalid ConfigEntry" << std::endl;
        TSelector::Abort("Invalid config entry",TSelector::kAbortProcess);
        return;
    }
}

PPCoincMapRegions::PPCoincMapRegions(PPCoincMapRegions const& obj)
    : fPed(),
    fThresh(),
    fLowCut(),
    fHighCut(),
...
{
#ifdef DEBUG
    std::cout << "Copy constructing PPCoincMapRegions" << std::endl; std::cout.flush();
#endif
...
void PPCoincMapRegions::SafeDeleteTH2D(TH2D* hist)
{
    if (hist!=NULL) delete hist;
    hist=0;
}

void PPCoincMapRegions::SafeDeleteTH1D(TH1D* hist)
{
    if (hist!=NULL) delete hist;
    hist=0;
}
...
PPCoincMapRegions::~PPCoincMapRegions()
{
#ifdef DEBUG
...
//_________________________________________________________________
void PPCoincMapRegions::Begin(TTree* /*tree*/)
{
#ifdef DEBUG
...
void PPCoincMapRegions::PrintBoundaryInfo(void)
{
...
//_________________________________________________________________
void PPCoincMapRegions::SlaveBegin(TTree* /*tree*/)
{
#ifdef DEBUG
    ...
    {
        ss.str("mult"); ss << it->first;

        sst.str("Multiplicity "); sst << it->first << " Events;Multiplicity Mask; Counts";
...
//_________________________________________________________________
void PPCoincMapRegions::Init(TTree* tree)
{
#ifdef DEBUG
...
//_________________________________________________________________
Bool_t PPCoincMapRegions::Process(Long64_t entry)
{
...
std::string PPCoincMapRegions::GenerateMask(std::vector<UInt_t>& vec)
{
...
UInt_t PPCoincMapRegions::ComputeRegion(UInt_t det_index, Double_t adc_val)
{
    UInt_t reg = 0;
...
UInt_t PPCoincMapRegions::FindNonZeroBin(std::vector<Bool_t> &vec)
{
    UInt_t index = 0;
...
//_________________________________________________________________
void PPCoincMapRegions::SlaveTerminate()
{
...
//_________________________________________________________________
void PPCoincMapRegions::Terminate()
{
    std::cout << std::endl;
...
void PPCoincMapRegions::LoadSettings(ConfigEntry *dbentry)
{
...
#ifdef VERBOSE_PPCoincMapRegions
