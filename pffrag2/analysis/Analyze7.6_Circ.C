...
TString correction_id = "238U_232Th_7.6_Circ";
TString bgnd_id = "238U_232Th_bgnd0";
TString calib_file = "eg7.6_Circ_calib.dat";
TString bgnd_calib_file = "overnight_bgnd0_calib.dat";

TString shift_data = "shifts_7.6_Circ_to_run667.dat";

Double_t norm = 16042.0 / 20485.5; // (total 10Hz clock time prod. data)/(total 10Hz clock time bgnd)

void Analyze7p6_Circ(bool force_recompile=true)
{
...
//     }
    if (OkToContinue("Calibrate corrected spectra"))
    {
        ...
        if (dir==0)
        {
            std::cout << "\tfailed to find bgsubbed_shifted_ltf_corr_adc_gt_thresh folder in " << f->GetName() << std::endl;
        }
        else
        {
            std::cout << "\tCalibrating bgsubbed_shifted_ltf_corr_adc_gt_thresh with " << calib_file
                    << std::endl;
            CalibrateHists(dir, calib_file.Data());
        }
        f->Close();
