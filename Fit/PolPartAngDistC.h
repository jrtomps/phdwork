#ifndef PolPartAngDistC_H
#define PolPartAngDistC_H
...
/*!
  ...
  */
class PolPartAngDistC : public ROOT::Math::IParametricFunctionMultiDim, public TFoamIntegrand
{
...
public:
    ...
    /*! The parameters are initialized to 1 and the range is set to be x[0] = [-1,1]
      ...
      */
    PolPartAngDistC(Double_t polarization=1.0,  POLARITY polarity=ELECTRIC);
    ...
    /*! The same initializations occur as in the default constructor but the
      ...
      * @param d is p[1]
      */
    PolPartAngDistC(Double_t c, Double_t d, Double_t polarization=1.0,  POLARITY polarity=ELECTRIC);
...
//      */
//    PolPartAngDistC(const ::Parameters& params, Double_t polarization=1.0,  POLARITY polarity=ELECTRIC);

    //! Copy constructor
    PolPartAngDistC(const PolPartAngDistC& rhs);
    //! Assignment operator
    PolPartAngDistC& operator=(const PolPartAngDistC& rhs);

    virtual ~PolPartAngDistC();

    virtual PolPartAngDistC* Clone() const;
...
private:
    ...
    ClassDef(PolPartAngDistC,0);

};

#endif // PolPartAngDistC_H
