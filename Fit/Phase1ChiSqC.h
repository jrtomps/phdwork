#ifndef Phase1ChiSqC_H
#define Phase1ChiSqC_H
...
class Phase1ChiSqC : public ROOT::Math::IBaseFunctionMultiDim
{
...
public:
    //! Constructor
    /*!
      * Construct the Phase1ChiSqC with a TGraph2DErrors pointer,
      ...
      */
    Phase1ChiSqC(TGraph2DErrors* gr,
                const ROOT::Math::IParametricFunctionMultiDim& weighted_angdistc,
                const std::vector<TH2*>& solang_pdfs);

    //! Copy constructor
    Phase1ChiSqC(const Phase1ChiSqC& obj);

    //! Assignment operator
    /*! uses the copy-and-swap idiom */
    Phase1ChiSqC& operator=(Phase1ChiSqC obj);

    //! swap function to implement the copy-and-swap idiom
    friend void swap(Phase1ChiSqC& lhs, Phase1ChiSqC& rhs);

    //! Destructor
    virtual ~Phase1ChiSqC();
    ...
    virtual Phase1ChiSqC* Clone() const;
...
private:
    ...
    std::vector<Double_t> NormalizeParameters(const Double_t* pars) const;
};


inline void swap(Phase1ChiSqC &lhs, Phase1ChiSqC &rhs)
{
    ...
    swap(lhs.fSA_PDFs, rhs.fSA_PDFs);
    swap(lhs.fPointToRegionMap, rhs.fPointToRegionMap);

}

#endif // Phase1ChiSqC_H
