...
UInt_t Phase2ChiSqYNCalls = 0;

Phase2ChiSqY::Phase2ChiSqY(TGraphErrors *gr,
...
Phase2ChiSqY::Phase2ChiSqY(const Phase2ChiSqY &obj)
...
Phase2ChiSqY& Phase2ChiSqY::operator=(Phase2ChiSqY obj)
{
    swap(*this, obj);
    return *this;
}

Phase2ChiSqY::~Phase2ChiSqY()
{
        delete fComputedRatios;
}

void Phase2ChiSqY::SetUpMap()
{
...
void Phase2ChiSqY::ConstructTheIntegrands()
{
  ...
  //      std::cout << fLambdas[i]->GetName() << std::endl;
        ...
        if (h.get()==0) throw std::runtime_error("in Phase2ChiSqY::ConstructTheIntegrands() a pdf is not a TH2");
        h->SetDirectory(0);
...
Phase2ChiSqY* Phase2ChiSqY::Clone() const
{
    return new Phase2ChiSqY(*this);
}

UInt_t Phase2ChiSqY::NDim() const
{
    return fUnpolAngDist->NPar();
}

std::shared_ptr<ROOT::Math::IntegratorMultiDim>& Phase2ChiSqY::GetIntegrator()
{
    return fIntegrator;
}

void Phase2ChiSqY::SetIntegrator(const std::shared_ptr<ROOT::Math::IntegratorMultiDim> &integrator)
{
    fIntegrator = integrator;
}

const std::map<UInt_t, Phase2ChiSqY::SSDPair>& Phase2ChiSqY::GetPointToRegionMap() const
{
    return fPointToRegionMap;
}

std::shared_ptr<ROOT::Math::IBaseFunctionMultiDim> Phase2ChiSqY::GetNumerator(UInt_t i)
{
    return fNumIntegrands.at(i);
}

std::shared_ptr<ROOT::Math::IBaseFunctionMultiDim> Phase2ChiSqY::GetDenominator(UInt_t i)
{
    return fDenomIntegrands.at(i);
}

void Phase2ChiSqY::SetAllIntegrandParameters(const Double_t *pars) const
{
...
//void Phase2ChiSqY::SetPointToRegionMap(const std::map<UInt_t, SSDPair>& p2rmap)
//{
//    fPointToRegionMap = p2rmap;
//}

std::vector<Double_t> Phase2ChiSqY::GetComputedRatios() const
{
    return *fComputedRatios;
}


Double_t Phase2ChiSqY::DoEval(const double *pars) const
{
    ...
    std::cout << "Call=" << std::setw(3) << Phase2ChiSqYNCalls << " p[0]=" << pars[0] << " p[1]=" << pars[1] << " p[2]=" << pars[2] << " p[3]=" << pars[3];
    std::cout << std::endl;
    ...
    ProgressBar pb(fY.size(), 32, std::cout, "  ");
...
void Phase2ChiSqY::CopyDataFromGraph(TGraphErrors* gr)
{
    if (gr==0)
    {
...
void Phase2ChiSqY::RemoveZeroesFromData(void)
{
