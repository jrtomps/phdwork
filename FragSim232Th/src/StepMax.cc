...
StepMax::StepMax(const G4String& processName)
 : G4VDiscreteProcess(processName),MaxChargedStep(DBL_MAX),
   fTargetName("av_1_impr_7_target_log_pv_0")
{
                                                  ...
                                                  G4ForceCondition* condition )
{
  ...
  *condition = NotForced;
  ...
  G4String aname = aTrack.GetVolume()->GetName(); 
  if( (MaxChargedStep > 0.) &&
      (aTrack.GetVolume() != 0) &&
      ((aname == fTargetName) 
        || (aname == "av_1_impr_8_TargetInsertBottom_LV_pv_2")
        || (aname == "av_1_impr_8_TargetInsertTop_LV_pv_2")) )
