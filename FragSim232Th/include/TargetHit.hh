...
*/
  ...
  void SetLocalPosition(const G4ThreeVector& vec);
...
private:  
  ...
  G4ThreeVector fdeltaWorldPos; //!< World particle displacement
  ...
  G4double            ftotDepE; //!< Deposited energy in the detector
...
public:
   ...
   */
  G4double GetPhi(void)      const {return fphi;     };
  //! Normal member function
  /*!
   * Returns the polar angle in unis of degrees
   */
  G4double GetTheta(void)    const {return ftheta;   }
  G4ThreeVector GetPosition(void) const {return finitWorldPos;}
  G4ThreeVector GetDeltaPosition(void) const {return fdeltaWorldPos;}
  G4ThreeVector GetMomentum(void) const {return finitMomentum;}
  ...
  G4ThreeVector GetLocalPosition(void) const {return flocalPos;}
};
...
inline void swap(TargetHit &lhs, TargetHit &rhs)
{
    ...
    swap(lhs.ftrackLength, rhs.ftrackLength);
