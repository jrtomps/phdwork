...
Eta2C* Eta2C::Clone() const
{
    return new Eta2C(fFile);
}

UInt_t Eta2C::NDim() const
{
...
TH2* Eta2C::Eta2CDAtTheta(UInt_t Z, UInt_t A, Double_t theta)
{
    TH3* h = GetTH3(Z, A);
    if (h==0) throw std::runtime_error("In Eta2C:Eta2CDAtTheta() null TH3 found");
...
TH2* Eta2C::Eta2CDAtPhi(UInt_t Z, UInt_t A, Double_t phi)
{
    TH3* h = GetTH3(Z, A);
    if (h==0) throw std::runtime_error("In Eta2C:Eta2CDAtPhi() null TH3 found");
...
TH2* Eta2C::Eta2CDAtDepth(UInt_t Z, UInt_t A, Double_t depth)
{
    TH3* h = GetTH3( Z, A);
    if (h==0) throw std::runtime_error("In Eta2C:Eta2CDAtDepth() null TH3 found");
...
TH3* Eta2C::GetAngleVsDepth(UInt_t Z, UInt_t A) const
{
    return GetTH3(Z,A);
}

UInt_t Eta2C::GetNIsotopes() const
{
    UInt_t count=0;
...
TH1* Eta2C::GetMassDistribution() const
{
   return dynamic_cast<TH1*>(fFile->FindObjectAny("mass_distribution"));
}

TH1* Eta2C::GetTKEDistribution() const
{
    return dynamic_cast<TH1*>(fFile->FindObjectAny("tke_distribution"));
}

Double_t Eta2C::GetNormalization() const
{
    return fNorm;
}

Bool_t Eta2C::IsIsotopeFolder(TDirectory *dir) const
{
    Bool_t flag = false;
    if (dir!=0)
    {
...
TH3* Eta2C::GetTH3(UInt_t Z, UInt_t A) const
{
    if (fFile==0) throw std::runtime_error("In <Eta2C::GetTH3()> null file argument");
...
void Eta2C::EnforceProbabilities()
{
//    fNorm = 1;
//    TH1* h = 0;

//    h = GetMassDistribution();
//    if (h==0) throw std::runtime_error("in Eta2C::GetNormalization() no mass distribution found");
...
Double_t Eta2C::ComputeIntegral(TH3* h)
{
    ...
    Double_t xlow[] = {-1.0, -1.0*CLHEP::pi};
    Double_t xhi[] = {1.0, CLHEP::pi};
    Double_t integ = integrator.Integral(xlow,xhi);

    return integ/prob_escape;
}

Double_t Eta2C::GetEnergyWeight(Double_t E) const
{
    // needs to finished
    return 1.;
}

Double_t Eta2C::DoEval(const double *x) const
{
    // x[0] = Z;
    // x[1] = A;
    // x[2] = CosTheta;
