...
void PrintMinosError(ROOT::Minuit2::Minuit2Minimizer& min)
{
    ...
    Double_t el, eh;
    std::vector<std::pair<Double_t, Double_t> > res(min.NDim());
    std::cout << std::setprecision(4) << std::setiosflags(std::ios::scientific);
    for (UInt_t i=0; i<min.NDim(); i++)
    {
        min.GetMinosError(i, el, eh);
        std::cout << "\n" << std::setw(3) << i
                << std::setw(12) << el
                << std::setw(12) << eh;

        res[i] = std::make_pair(el,eh);

    }
    std::cout << std::endl;

    std::cout <<"\n RESULTS OF MINOS ERROR ESTIMATOR" << std::endl;
    for (UInt_t i=0; i<res.size(); i++)
    {
        std::cout << "\n" << std::setw(3) << i
                << std::setw(12) << res[i].first
                << std::setw(12) << res[i].second;
    }
    std::cout << std::endl;
}


void PrintGradient(ROOT::Minuit2::Minuit2Minimizer& min)
{
...
//        throw std::runtime_error("Invalid polarization value!");
    ...
    AngDistY w(params, 0, multipol);//, -5.0*TMath::DegToRad());


    PolPartAngDistY polpart(1, multipol);//, -5.0*TMath::DegToRad());
   ...
   min.SetErrorDef(1.0); // this is the up
