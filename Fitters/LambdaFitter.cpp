...
void DisplayAllFilesInDirOfType (std::string dr, std::string suffix)
{
    ...
    std::cout << std::left
            << "\nThe following " << suffix << " file are in the current folder\n";
...
int NumericalMinimization()
{
    ...
    // Step #2 Get the lambda functions
    std::string hfname;
    std::cout << "\nFor the lambdas pdfs ";
    DisplayAllFilesInDirOfType (".",".root");
    std::cout << "\nEnter filename : ";
...
//    Eta2 eta(feta);
//    AngMarginalEta2 marg_eta(eta);
//
//    WeightedAngDist w(params, marg_eta, pol,multipol);
    AngDistC w(params, pol, multipol);


    LambdaChiSq FUNC(mygr, w, solang_hists);
